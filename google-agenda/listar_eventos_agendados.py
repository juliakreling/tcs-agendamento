...
ID_CALENDARIO_AGENDADOS = '812bfe549dd5ab2c747bac7874a0c500103766d05ce9d03a808bc9769fbefa5b@group.calendar.google.com'


class ClasseListarEventosAgendados():

    def listar_eventos_agendados(self, qtde_dias: float) -> list:
        creds = None
        ...
        try:
            ...
            while True:
                events = service.events().list(
                    calendarId=ID_CALENDARIO_AGENDADOS,
                    timeMin=time_min,
                    timeMax=time_max,
    ...
    def formatar_eventos_agendados(self, qtde_dias: float) -> list:
        nao_formatados = self.listar_eventos_agendados(qtde_dias)
        lista_formatada = []
...
listar_eventos = ClasseListarEventosAgendados()
app = Flask(__name__)


@app.route('/listar_eventos_agendados/<float:parametro>', methods=['GET'])
def obter_eventos_agendados(parametro):
    eventos_formatados = listar_eventos.formatar_eventos_agendados(parametro)
...
# http://127.0.0.1:5004/listar_eventos_agendados/2.0
